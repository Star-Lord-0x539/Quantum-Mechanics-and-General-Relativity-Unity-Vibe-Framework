...
% only; U_omega (H^s(K'')-precompact/nowhere dense, empty H^s-interior); s>11 / s>23/2.
%
...
here and respected throughout Parts~2--3, and \emph{stronger} than the
interior-emptiness recorded in earlier passes: on the slab $U_\omega$ is
$H^s(K'')$-\emph{precompact} (run the Cauchy-estimate step of
Lem.~\ref{an17:lem-embed} at any order $m>s$ and apply Rellich), hence nowhere
dense in $H^s$ with empty $H^s$-interior; so every uniformity claimed over it is
a supremum over the ball or a pair-Lipschitz modulus, never an $H^s$-open
property. What \emph{is} $H^s$-dense (in the $C^0$-tube of $g_0$) is the
analyticity restriction as such --- the union of the collar classes over all
$\rho_a>0$ --- by heat-flow/entire-function mollification (chartwise
mollification with a partition of unity destroys analyticity; standard fact,
consumed nowhere below); every theorem of this appendix is uniform over each
\emph{fixed} class. This is exactly the
scope in which the paper carries Hyp.~\ref{hyp:hadamard-uniform-omega} (Option~A,
the analytic-collar restatement).
\end{remark}

\begin{lemma}[strengthening of the disclosed cost: precompactness on the
slab]\label{an17:lem-precompact}
$U_\omega|_{K''}$ is $H^s(K'')$-precompact, and its $H^s(K'')$-closure lies in
the $\le$-relaxed collar class on each strictly smaller collar: every limit
point is holomorphic on $\mathcal K_{\rho}$ for each $0<\rho<\rho_a$ with
$\|g-g_0\|_{\mathcal K_{\rho}}\le\varepsilon_a$.
\end{lemma}
\begin{proof}
Fix any order $m>s$: the Cauchy-estimate step of Lem.~\ref{an17:lem-embed}, run
at order $m$, bounds $U_\omega$ in $H^m(K'')$ by $C_{\mathrm{emb}}(m)\,
\varepsilon_a$, and Rellich ($m>s$, $K''$ compact) compresses the bounded
$H^m(K'')$-set into an $H^s(K'')$-precompact one. Montel identifies the closure:
an $H^s(K'')$-convergent sequence in $U_\omega$ is uniformly bounded on the
collar, so a subsequence converges locally uniformly on its interior to a
holomorphic limit, and the sup bound survives as $\le\varepsilon_a$ on each
$\mathcal K_{\rho}$, $0<\rho<\rho_a$ (the class's strict $<$ need not survive
the limit --- closedness holds under $\le$). This abstract compactness is
recorded as disclosure only; the quantitative route of record remains the
collar margin with its explicit constants (Rem.~\ref{an17:rem-topology}):
abstract compactness and attainment yield no numbers.
\end{proof}
...
\begin{equation}\label{an17:eq-MGamma}
 M_\Gamma\;:=\;\max\Bigl(\tfrac12,\ \sup_{g\in U_\omega}\|\Gamma[g]\|_{\mathcal K_{\rho_a/2}}\Bigr)\;\le\;
 \tfrac12+c_n\,\rho_a^{-1}\bigl(\|g_0\|_{\mathcal K_{\rho_a}}+\varepsilon_a\bigr)
 \bigl(1+d_\omega^{-1}\bigr)^{n}\;<\;\infty
\end{equation}
(one Cauchy derivative costs $\rho_a^{-1}$; $\partial g$ and $g^{-1}=(\det g)^{-1}
\operatorname{adj}g$ holomorphic by Lemma~\ref{an17:lem-embed}; the $\max$ with
$\tfrac12$ is a WLOG normalisation: $M_\Gamma$ is consumed downstream only as an
upper bound on $\|\Gamma\|$ and as a formal parameter of the derived constants
$r_t,M_\phi'',\mathcal R_0,N_0,\sigma_\dagger,C_{\mathrm{rim}}'$, each entered
upper-bound-safely, and it secures the collar-containment margins of (ii) and
Prop.~\ref{an17:prop-strip} uniformly --- closing the corner $M_\Gamma<\tfrac14$
(near-flat anchor, thin collar) in which the containments printed before this
normalisation failed); \emph{(ii)} for
...
$\le\tfrac12$; containment: the drift is $\le2|p|\cdot\tfrac14\le
\rho_a/(16M_\Gamma)\le\rho_a/8$ by the normalisation $M_\Gamma\ge\tfrac12$), and,
by time-homogeneity $\gamma_p(\lambda\tau)=\gamma_{\lambda p}(\tau)$, the same
statement holds at any momentum cap $P>0$ on the disc
$\{|\tau|\le\rho_a/(32M_\Gamma P)\}$. References: Hille, \emph{ODE in the Complex Domain}, Thm~2.2.2;
...
\begin{equation}\label{an17:eq-rt}
 r_t\;:=\;\min\!\Bigl(\tfrac14,\ \tfrac{\rho_a}{64\,M_\Gamma\,c_{\mathrm{ch}}
 \rho_0}\Bigr),
\end{equation}
...
(Remark~\ref{an17:rem-CK}). Applying the momentum-rescaled (ii) at each base
point $\gamma(t_0)$, $t_0\in[0,1]$, at the cap $P=2c_{\mathrm{ch}}\rho_0$ extends
the flow to a complex-time disc of $t_0$-uniform radius
$\tau_0'=\rho_a/(64M_\Gamma c_{\mathrm{ch}}\rho_0)$, with drift
$\le\rho_a/(16M_\Gamma)\le\rho_a/8$ ($M_\Gamma\ge\tfrac12$), inside the collar
where $\|\Gamma\|\le M_\Gamma$ \textup{[constant corrected: the previously
printed radius $\rho_a/(16M_\Gamma c_{\mathrm{ch}}\rho_0)$ overdrew (ii)'s
momentum--radius budget by the absolute factor $4$ and, for $M_\Gamma<\tfrac14$,
exited the collar; no consumer reads the numeric $r_t$, so the correction moves
no register]}; the discs glue by uniqueness of continuation to the strip
...
some tile has strip-sup $<m_0$ (the phase is near-flat on that tile --- the
other tiles may still carry strip-sup up to $M_\phi''$) and the count is not
...
holds on the fold stratum $\mathcal N_{m_0}$ for all $\mu$, and on all of
$\Sigma_y$ for $\mu\ge\kappa_V$ (a fixed ball-uniform threshold) with a
ball-uniform inflated constant (Part~2, Thm.~\ref{an17:thm-linear}), with a
count-free $\mu^{1/2}$ remainder confined to the sliver regime $\mu<\kappa_V$,
where the per-tile floor fails on some tile and the count is not invoked (the
fibre may still fold on its loud tiles --- no whole-interval
$\phi''$-smallness is implied); whether (D3) then closes over $U_\omega$ is Part~2's cascade, not a
...
\end{equation}
with the ball-uniform, $\bar\kappa_2$-\emph{free} constant of record, instantiated
from the in-file cell ledger,
\begin{equation}\label{eq:an17-C2}
 C_2\;:=\;C_{\mathrm{part}}\;+\;8\,c_{\mathrm{fan}}c_w^2
 \;=\;2^{12}c_w^2\bigl(c_{\mathrm{fan}}+c_{\mathrm{fold}}\bigr)
 +8\,c_{\mathrm{fan}}c_w^2
\end{equation}
($C_{\mathrm{part}}$ of \eqref{eq:an17-Cpart} collecting the four cell lines of
Lemma~\ref{an17:lem-ledger}; $8c_{\mathrm{fan}}c_w^2$ the endpoint $B_1$-mass of
the proof below; on the consumed sublevel route, route-S,
$c_{\mathrm{fold}}=c_{\mathrm{sub}}^\star$ and no $\phi'''$-floor fraction
enters, the count-route $c_{\mathrm{fold}}=(1+N_0)^2\eta^{-2/3}$ being
corroborating only). \textup{[Display correction: the earlier form of this
constant was assembled from per-cell symbols
$A_{\mathrm{II}},A_{\mathrm{fold}},A_{\mathrm{bad}},c_\tau$ quoted from ancillary
worksheets that are not part of the shipped corpus, and its $c_\tau^{-2/3}$
factor was a $\phi'''$-floor vestige of the count route that the consumed chain
does not take; the display above replaces it by the in-file instantiation --- a
bookkeeping reconciliation, not a re-pricing: splitting device powers between
$c_{\mathrm{fold}}$ and $D^2$ redistributes but never lowers the composite
$C_2D^2$, and the consumed headline $C_2(1+N_0)^2$ of
Cor.~\ref{an17:cor-A1ledger}/Prop.~\ref{an17:prop-D3} dominates any such
distribution.]} $C_2$ depends only on $(U_\omega,K_\Sigma,n,c_w)$ through the
ledger constants {\rm(}all $\bar\kappa_2$-free{\rm)}, and is independent of
$y,e,\rho,\Lambda$ and of $\bar\kappa_2$. The device factor $D^2$ rides the
vdC-II shells \eqref{eq:an17-II} (and, on the count route only, the fold cell
through $c_{\mathrm{fold}}$); the rim, bad, and endpoint masses are $D$-free ---
the bad cell in particular is priced by the trivial cap against the F2 measure
with no device input \eqref{eq:an17-bad}.
\end{theorem}
\begin{proof}
...
pure linear device bound $d\theta(S_\mu)\le(c_{\mathrm{sub}}
+c_{\mathrm{fan}}(\mathcal R_0\kappa_b+2))\mu$ for $\mu\ge
\kappa_V:=m_0/(\bar\kappa_2\rho^2)$ ($=O(1)$ for $\rho\ge\rho_{\min}$); the (D3) bad cell, however, is fed at
...
Part~1: some tile has strip-sup $<m_0$) only the count-free
...
\begin{equation}\label{eq:an17-supbound}
 ...
 =O(1),
\end{equation}
and consequently $|\phi'(t;\theta)-e\cdot v|\le M_{\phi''}$ (mean-value anchor,
see the proof), so
...
Part~1 ratio bound. For the $\phi'$-window, anchor by the mean value theorem:
$q(0)=q(1)=0$ (R1) gives $\phi(1)-\phi(0)=e\cdot v$, so $\phi'(\bar t\,)=e\cdot v$
exactly for some $\bar t\in[0,1]$, whence $|\phi'(t)-e\cdot v|=
|\int_{\bar t}^{t}\phi''|\le M_{\phi''}$. \textup{[Proof-line correction: the
identity $\phi'(0)=e\cdot v$ implicitly used before is false ($\phi'(0)=e\cdot v
+e\cdot q'(0)$; only $q(0)=q(1)=0$, $\partial_yq(0)=0$ are exports), and the
former clause absorbing an $O(\bar\kappa_2\rho^2)$ endpoint drift is deleted ---
it compared the incomparable sup norms $K_\Gamma$ and $M_\Gamma$. The stated
$\pm M_{\phi''}$ window is unchanged.]}
\end{proof}
...
bound $O(D^2\mu_\star)$ relies on $D^2$ being LARGE (it counts the
...
\rho\sigma>0$, so $\phi'$ is single-signed and one IBP gives $|I_\Lambda|\le18b/(\Lambda
...
device measure bound $O(D^2\mu_\star)$ is unavailable below $\kappa_V$
...
\kappa_2\rho^2)=(c_0/(2\bar\kappa_2))(\rho_{\min}/\rho)^2$ (so $\le c_0/(2\bar
\kappa_2)=O(1)$ for $\rho\ge\rho_{\min}$; for $\rho<\rho_{\min}$ it grows like
$\rho^{-2}$ --- harmless, since a larger $\kappa_V$ only narrows the claimed
pure-linear range). Then for every $g\in U_\omega$,
$y\in K_\Sigma$, $0<\rho\le\rho_0$, $|e|=1$,
\begin{equation}\label{eq:an17-linear}
 d\theta(S_\mu)\le\bigl(c_{\mathrm{sub}}+c_{\mathrm{fan}}(\mathcal R_0\kappa_b
 +2)\bigr)\mu\ \ (\mu\ge\kappa_V),\qquad
...
\varepsilon_a,\dots)$. The pure linear bound holds with constant
$c_{\mathrm{sub}}+c_{\mathrm{fan}}(\mathcal R_0\kappa_b+2)$ for $\mu\ge\kappa_V$
on all of $\Sigma_y$, and with constant $c_{\mathrm{sub}}$ for \emph{all} $\mu>0$
on the fold stratum $\mathcal B_{\mathcal N}$ (the line the (D3) path consumes); the sole degradation is the count-free
...
($\mathcal B_{\mathcal V}$).} Here \emph{some tile} carries strip-sup $<m_0$
(Def.~\ref{an17:def-strata}); the whole-interval sup may still reach
$M_\phi''=\mathcal R_0m_0$ on the other tiles (mixed fibres), so no
whole-interval flatness is assumed; the count is not invoked. Two facts (Cor.~an-split(b1)--(b2)), not their
unqualified union: (b1) the count-free $d\theta(S_\mu\cap\mathcal B_{\mathcal V})\le
C_{\mathrm{sub}}\max(\mu,\rho\mu^{1/2})$ for all $\mu>0$
(Theorem~\ref{an17:thm-Csub}); (b2) for $\mu\ge\kappa_V$ a $\sigma$-band estimate: by
Lemma~\ref{an17:lem-flat} the $t$-span of $\Xi(\cdot,\theta)$ is $\le
M_\phi''/\rho=\mathcal R_0\kappa_V\bar\kappa_2\rho\le\mathcal R_0\kappa_b\,\mu$
(using $m_0=\kappa_V\bar\kappa_2\rho^2$ and $\bar\kappa_2\rho\le\kappa_b$), so by
the equatorial band mechanism of Lemma~\ref{an17:lem-capfails} (the
$\sigma$-shift structure and two-sided F2) $S_\mu\cap\mathcal B_{\mathcal V}$
lies in a $\sigma$-band of width $\le(\mathcal R_0\kappa_b+2)\mu$, of measure
$\le c_{\mathrm{fan}}(\mathcal R_0\kappa_b+2)\mu$. \textup{[Correction: the
former (b2) declared the $\phi''$-branch inactive and priced the pure velocity
sublevel at $c_{\mathrm{vel}}\mu$; on mixed fibres of $\mathcal B_{\mathcal V}$
that inference fails as written, and the constant is repaired to
$c_{\mathrm{fan}}(\mathcal R_0\kappa_b+2)$.]} For $\mu<\kappa_V$ only
...
$d\theta(S_\mu)\le(c_{\mathrm{sub}}+c_{\mathrm{fan}}(\mathcal R_0\kappa_b+2))\mu$
(this whole-$\Sigma_y$ line is consumed by nothing on the operative (D3) chain,
which uses Part~A on $\mathcal B_{\mathcal N}$ and A1 on
$\mathcal B_{\mathcal V}$; the (b2) constant inflation is confined to this
display); for $0<\mu<\kappa_V$, Part~A gives
...
%    U_omega (H^s(K'')-precompact/nowhere dense, interior-empty, sup/pair-Lipschitz only); s>11 slice /
...
pair $|a|=|b|=1$ only); the class $U_\omega$ is $H^s(K'')$-precompact ---
nowhere dense, with empty $H^s$-interior --- so every uniformity below is a
supremum over the ball or a pair-Lipschitz modulus.
\end{remark}
...
$L^2_x$ by Minkowski using (D1)'s $e$-uniformity --- it is here, and only here,
that the fan mass normalisation enters, so the factor $C_0$ carries the (D1)
constant $C_{\mathrm{fan}}=\sqrt{15c_{\mathrm{fan}}}\,c_w+\sqrt{C_2}\,D
\supseteq(1+N_0)^{1/2}$: $C_{\mathrm{Schur}}$, not the dyadic factor $c_a$ of
\eqref{an17:eq-D5} (which is $C_{\mathrm{fan}}$-free as displayed), is the
$D$-carrier of the $S4$ leg. A $D$-free reading of
\eqref{an17:eq-D4}--\eqref{an17:eq-D5} would prove the booking over the naive
$H^s$ ball, contradicting the ripple no-go (Lemma~\ref{an17:lem-ripple}). No
per-$\theta$ quantity is
...
\begin{equation}\label{an17:eq-KT1}
 ...
 \qquad C_{\mathrm{Schur}}\ \text{built from}\ C_{\mathrm{fan}}\
 \text{(via $C_0$; $c_a$ is $C_{\mathrm{fan}}$-free)},
\end{equation}
...
\begin{equation}\label{an17:eq-chain}
 ...
 C_{\mathrm{fan}}
 \ \longrightarrow\
 C_{\mathrm{Schur}}\ (\text{via }C_0,\ \text{Thm.~\ref{an17:S4a-D4}})
 \ \longrightarrow\
...
$H^s(K'')$-\emph{precompact} --- nowhere dense, with \emph{empty}
$H^s$-interior; downstream coercivity floors therefore use the collar-margin
attainment note (the infimum over $U_\omega$ is controlled by the collar
margin, \emph{not} by attainment on a closed $H^s$-ball, which fails).
\end{remark}
...
\texttt{s4b\_arith\_check.py} / \texttt{s4b\_schur\_check.py}, deposited as
ancillary files with the arXiv source (not needed to compile or to check
any proof). The record is a
...
the mixed-jet finite-part family $\{C_W,W_*\}$ (PLAUSIBLE: reduced to $s>15$ at
$N^\star{=}4$ via the $N{=}4$ truncated finite part --- the $N{=}2$ route is
obstructed at the seed data heights at every $s>11$, Rem.~\ref{an17:d0-record} ---
modulo the not-in-print $N{=}4$ family-uniform Hadamard construction (existence a
named premise), the un-executed multi-index constant-tracking, and the
$N^\star{=}4$ seed-data supply), and the Sanders QEI-constant uniformity (its
multiplicative $C_S$ leg now a theorem over $U_\omega$,
Lem.~\ref{lem:cs-CS-uniform}; its additive $c_S$ leg riding clause~(ii),
F-iii-fenced) --- remain, at those grades, the paper's external input,
...
$d(\sigma_g|_{\Sigma_0})(p)\neq0$, with slope bounded below, at each
\emph{fixed} $y$, by the $c_0,\delta_0$-data (the fixed-$y$ floor
degenerates as the vertex approaches the slice; the $y$-uniform statement
is carried by the two-zone estimate closing this proof). Hence in
$c_0$-uniform tubular coordinates
...
the level, as the vertex approaches the slice). Quantitatively: let
$\tau\in(0,u_0]$ be the vertex height above $\Sigma_0$ and split at
$|u|\asymp\tau$. On the transverse zone $|u|\gtrsim\tau$ the profile is the
one-dimensional model above with the degenerating slope $|e|\asymp\tau$ as
amplitude factor; the scaling count (amplitude $\times$ length$^{1/2}$
$\times$ frequency$^{t}$) bounds the zone-boundary shell $|u|\asymp\tau$ by
$C\,\tau^{\alpha+\frac12-t}|\log\tau|$ (at $\alpha=0$ the slope enters only
through the logarithm, leaving $C\,\tau^{\frac12-t}|\log\tau|$), and the
outer shells sum geometrically to the fixed-$y$ transverse bound already
proved. On the radial zone $|u|\lesssim\tau$ the profile is the radial
model $\rho^\alpha\log\rho$ of Lem.~\ref{an17:no-glancing}(ii) at scale
$\rho\asymp\tau$; the same count in the three transverse dimensions gives
$C\,\tau^{\alpha+\frac32-t}|\log\tau|$. For every $t<\tfrac\alpha2+\tfrac12$
both $\tau$-exponents are positive, so the supremum over $\tau\in(0,u_0]$
is finite and the degenerate limit contributes $0$: the patched bound is
$y$-uniform at the level $t<\tfrac\alpha2+\tfrac12$, with no loss from the
gluing.
\end{proof}
...
re-derivation --- now performed (the source-rung survival sweep of
Rem.~\ref{an17:d0-record}(v): every gated column is confirmed and the source
rung over-delivers in lockstep, imposing no upper fence); the arithmetic is
deliberately kept, not deleted:} the A1/A2
...
\item \emph{Consequence for (D0) (NEGATIVE-RESULT, sharp).} The seed supplies
  ...
  $s<11.5$ on the half-corrected spot-check (for all $s$ once the demand is
  priced honestly, item~(v)) and dies $0^+$ at its $\partial^2$-consumer). (D0)
  is OBSTRUCTED on the printed route at the seed truncation $N{=}2$ --- a real
  obstruction at the cone crossing, not a write-out gap; it is SUPPLIABLE at
  $N^\star{=}4$ once the source columns are re-derived (item~(v)).
\item \emph{Repair: LANDS at $N^\star{=}4$ (the source columns re-derived; the
  precondition of this record now met).} Raising the truncation $N$ buys one
  transverse order per unit $N$ on every data leg; the $k$-counts give $b{=}0$
  healing at $N{=}3$ and $b{=}1$ at $N\ge4$. The see-saw's reuse of the gated
  source columns (the slab register $p=\alpha+2$ is the vertex-zone height;
  near the crossing the honest slab profile of the $\sigma^2\log\sigma$ factor
  is $\tfrac52^-$, under which the $b{=}0$ source rung would survive only for
  $s<11.5$ --- the half-corrected number, spot-checked at $b{=}0$ only) is now
  discharged: the dedicated source-rung survival sweep has been carried out
  over every slab booking meeting the cone off-diagonal. Under the FULL honest
  pricing the source demand is the profiled $\partial^2$-package output, which
  drops in lockstep with the supply (componentwise margins $(\tfrac12,1)$ at
  every $N$, both $b$), so the source rung over-delivers for all $s$ and
  imposes no upper fence --- the $s<11.5$ ceiling was the supply-only
  half-correction. The genuine obstruction is the fixed-threshold race: the
  mixed $(1,1)$ coincidence profile $(N-\tfrac32)^-$ against the $\tfrac32$
  Morrey embedding, missed by one order at $N{=}2$ (the (D0) death of (iv),
  which stands at the seed), dead-$0^+$ at $N{=}3$, cleared with margin $1$ at
  $N{=}4$. The source columns thus re-derived, the repaired fence may now be
  quoted: the half-line $s>15$ in the slice register and $s>\tfrac{31}2$ in the
  locked-4d register ($=2N+7$ resp.\ $2N+\tfrac{15}2$ at $N^\star{=}4$,
  recovering the printed $N{=}2$ values $s>11,\ s>\tfrac{23}2$). Hence (D0) is
  SUPPLIABLE at $N^\star{=}4$ --- THEOREM at the seed given the corrected
  transverse rule, the ungated coefficient columns, and $N{=}4$ existence; the
  end-to-end close-out inherits the standing $B3/$Lem.~\ref{an17:Nc-corrected}
  difference-data interface at PLAUSIBLE and the family-uniform finite-part as
  a named input. The $N{=}2$ obstruction of (iv) is unchanged; the alternative
  repair direction (a consumer rewiring that stops feeding slice-Sobolev norms
  of the tail across crossing spheres) is not needed.
\end{enumerate}
\end{remark}

\begin{remark}[the survival-audit rows behind item (v); verifiability only]
Transcribed from the item-(v) sweep record in its $(c,p)$ register --- no new
claim, no re-derivation; the affine law is one transverse order per unit $N$ on
every data leg. Binding per-slot rows, supply pair vs demand coefficient:
$b{=}0$ source rung $(2N+4,\,N+\tfrac12)$ vs $2N+5$, supply-only ceiling
$3N+\tfrac{11}2$ --- $\tfrac{23}2$ at $N{=}2$ (the printed half-corrected
$s<11.5$), $\tfrac{29}2$ at $N{=}3$, $\tfrac{35}2$ at $N{=}4$; $b{=}1$ rung
$(2N+4,\,N-\tfrac12)$ vs $2N+\tfrac{11}2$, ceiling $3N+5$ --- $14$ at $N{=}3$,
$17$ at $N{=}4$ (tighter by $\tfrac12$ at every $N$). Under the full honest
pricing the demand is the profiled $\partial^2$-package output dropping in
lockstep, so the componentwise margins are $(\tfrac12,1)$ at every $N$, both
$b$: these ceilings are saturation loci, not fences (item (v)). What gates is
the race column alone: the mixed $(1,1)$ profile $(N-\tfrac32)^-$ against the
fixed $\tfrac32$ --- $\tfrac12$ at $N{=}2$ (missed by one), $\tfrac32^-$
(dead-$0^+$) at $N{=}3$, $\tfrac52$ (margin $1$) at $N{=}4$; fences $s>2N+7$,
resp.\ $s>2N+\tfrac{15}2$.
\end{remark}
...
(Rem.~\ref{an17:d0-record}), both at the seed truncation $N{=}2$; the repair
lands at $N^\star{=}4$ (Rem.~\ref{an17:d0-record}(v), source-rung sweep
discharged). The coefficient columns and the $s>11$
...
\]
ball-uniform \emph{by the collar margin, not by attainment}: the anchor floor
...
\item[A2.] \emph{Source, $b=1$ \textup{[Thm.~\ref{an17:T1prime}]}:}
  ...
  the promoted booking (tail hypothesis $a=s-8>2$, the
  Thm.~\ref{an17:T1prime} floor \textup{[corrected from the misquoted
  ``$>\tfrac32$: margin $\tfrac32+\delta$'', which understated the demand and
  would falsely license $s\in(9.5,10]$]}: margin over the true floor $1+\delta$
  at the $s>11$ fence, every consuming locus having $a>3$), plus $(\square_{g'}v'_2)(\partial_y\Pi)$ at $(8,5)$, plus
  ...
  rung would survive only for $s<11.5$ WERE the demand left at its gated
  coefficient register --- a half-correction. The dedicated sweep is now
  performed over every slab booking meeting the cone off-diagonal
  (Rem.~\ref{an17:d0-record}(v)): correcting the demand booking in lockstep
  with the supply --- both are $\sigma^2\log\sigma$-type vertex-zone profiles
  over-priced by the identical old rule, so both drop by the same amount and
  the componentwise margins are $(\tfrac12,1)$ at every $N$, both $b$ --- every
  source-side booking over-delivers, and the source rung imposes NO upper fence
  ($s<11.5$ was the half-corrected, supply-only number). The printed bookings
  and legality margins therefore STAND, now confirmed by the transverse
  re-derivation (Rem.~\ref{an17:transverse-note}); the Fubini inequality
...
over the $H^s$-interior-empty $U_\omega$ is needed --- a lower bound on an
infimum requires no minimiser; this is the honest replacement of the false
``$H^s$-closed ball, infimum attained'' clause. Only $1/\theta\le1/\theta_*$ is consumed
...
\item \textup{(D0)} --- \emph{the seed-data input.} The Cauchy-data levels the
  ...
  (Rem.~\ref{an17:d0-record}). At the seed $N{=}2$ the gate does NOT lift; the repair LANDS at $N^\star{=}4$
  (the source-rung survival sweep of Rem.~\ref{an17:d0-record}(v) now
  discharged --- the source columns re-derived, the source demand priced
  honestly as the profiled $\partial^2$-package output that drops in lockstep
  with the supply, so the source rung over-delivers for all $s$ and imposes no
  upper fence; (D0) is SUPPLIABLE for $s>15$ --- THEOREM at the seed,
  end-to-end PLAUSIBLE via the standing $B3$/(E-env) interface). The (D0) port's closure display
...
\emph{as a whole} is PLAUSIBLE (REDUCED, not proved) --- reduced to $s>15$ at
$N^\star{=}4$ via the $N=4$ truncated finite part (the $N=2$ route is obstructed
at the seed data heights at every $s>11$, Rem.~\ref{an17:d0-record}) --- because
the $N=4$ family-uniform finite-regularity Hadamard construction is not in print
(its existence a named premise), the full multi-index constant-tracking is not
executed, and the $N^\star{=}4$ seed-data supply is outstanding. Clause (ii) is therefore carried, here and in
...
\label{an17:CW-grade}
On a single convex-normal chart the mixed $(1,1)$ difference jet closes at $s>15$
at $N^\star{=}4$ via the $N{=}4$ truncated finite part (the $N{=}2$ route is
obstructed at the seed data heights at every $s>11$, Rem.~\ref{an17:d0-record};
$N{=}4$ existence a named premise), and the clause-(ii) constant assembles as
\[
...
$C_W/W_*$ PLAUSIBLE (Rem.~\ref{an17:CW-grade})\} alone; clause (iii) is now a
theorem modulo clause~(ii): its $C_S$ leg is proved over $U_\omega$
(Lem.~\ref{lem:cs-CS-uniform}), its $c_S$ leg riding clause~(ii) (the clause (i) difference form,
...
% ---------------------------------------------------------------------
\subsection{Clause (iii): the QEI-constant uniformity (a theorem modulo
clause~(ii), at its audited grade)}
...
record its grade exactly.

\begin{remark}[Clause~(iii) is a theorem modulo clause~(ii); its $C_S$ leg is
proved in print, its $c_S$ leg rides clause~(ii)]\label{rem:an17-clauseiii}
...
matter of constant-tracking rather than new analysis''.  The multiplicative
($C_S$) half of this tracking \emph{is} now derived in print over $U_\omega$
(Lem.~\ref{lem:cs-CS-uniform}); only the additive ($c_S$) half rides clause~(ii).  The companion chain's only contact with clause~(iii) is the
...
hypothesis.  \emph{Grade: THEOREM modulo clause~(ii): the multiplicative ($C_S$)
constant-tracking is derived (Lem.~\ref{lem:cs-CS-uniform}); the additive
($c_S$) leg rides clause~(ii), and the F-iii subspace-topology scoping is
...
Lipschitz bound closes at $s>15$ at $N^\star{=}4$ via the $N=4$ truncated
Hadamard finite part (the $N=2$ route is obstructed at the seed data
heights at every $s>11$, Rem.~\ref{an17:d0-record}), with $C_W(K)$
assembled from named ball-uniform factors; but it is \emph{reduced, not
proved}, modulo three honest gaps -- the $N=4$ finite-regularity
Hadamard-parametrix construction with quantitative $v_4$, family-uniform
in $g$, is not in print as a ball-uniform statement (the same status the
paper already grants the $N=1$ construction; its existence a named
premise), the exact multi-index constant-tracking through the $N=4$
commutator source and the energy estimate is not executed, and the
$N^\star{=}4$ seed-data supply (the re-derived source columns,
Rem.~\ref{an17:d0-record}(v)) is outstanding.
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=*]
  ...
  \item \emph{The class is $U_\omega$, not an open $H^s$ ball.}
    $U_\omega$ is $H^s(K'')$-precompact --- nowhere dense, with
    \emph{empty} $H^s$-interior (the disclosed cost, stronger than
    interior-emptiness alone).  Every uniformity in
...
\begin{enumerate}[label=\textup{(\alph*)},leftmargin=*]
  \item \emph{The three named clauses themselves.}  (M1) clause~(ii) is
    PLAUSIBLE (reduced to $s>15$ at $N^\star{=}4$ modulo the $N=4$
    Hadamard construction --- the $N=2$ route obstructed at the seed,
    Rem.~\ref{an17:d0-record} --- the constant-tracking, and the
    seed-data supply); (M2) clause~(iii)
    is a theorem modulo clause~(ii) ($C_S$ leg proved,
    Lem.~\ref{lem:cs-CS-uniform}; $c_S$ rides clause~(ii)); (M3)
...
\begin{enumerate}[label=\textup{(M\arabic*)},leftmargin=*]
  \item\label{item:an17-M1} \textbf{Clause (ii) -- finite part $W_g$
    ...
    W_*(K)$, ball-uniform.  Reduced to $s>15$ at $N^\star{=}4$ via the
    $N=4$ truncated Hadamard finite part (the $N=2$ route is obstructed
    at the seed, Rem.~\ref{an17:d0-record}(v); $N=4$ existence a named
    premise --- the correction gate below) with $C_W(K)$ assembled from
    named ball-uniform factors, \emph{modulo two honest gaps not closed}:
    (a) the $N=4$ finite-regularity Hadamard-parametrix construction with
    quantitative $v_4$, family-uniform in $g$, is not in print as a
    ball-uniform statement; (b) the exact multi-index constant-tracking
    through the $N=4$ commutator source and the energy estimate is not
    executed.
    ...
    so (D0) is a confirmed obstruction of the printed route at the seed $N{=}2$,
    SUPPLIABLE at $N^\star{=}4$ once the source columns are re-derived
    (Rem.~\ref{an17:d0-record}(v); the survival sweep discharged, the half-line
    $s>15$).
  ...
  \item\label{item:an17-M2} \textbf{Clause (iii) -- Sanders
    ...
    $H^s$ topology (Fence F-iii); the multiplicative ($C_S$) constant-tracking is now derived in print
    (Lem.~\ref{lem:cs-CS-uniform}), the additive ($c_S$) leg riding clause~(ii).  \emph{Consumer:}
    ...
    \emph{Grade: THEOREM modulo clause~(ii)}
    (the $C_S$ leg is derived, Lem.~\ref{lem:cs-CS-uniform}; the $c_S$ leg
    rides clause~(ii), and the companion touch is the F-iii scoping repair).
...
patches; $(D0)$ is OBSTRUCTED on the printed route at the seed $N{=}2$, SUPPLIABLE
at $N^\star{=}4$, Rem.~\ref{an17:d0-record}), the \#A collar architecture (the long-slab
