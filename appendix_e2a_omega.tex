...
(Rem.~\ref{an17:calibration}) reproduce the data wall exactly --- but the
...
sketch at the same grade as the $N_0$ collar-margin argument of Part~1
...
\begin{enumerate}[label=\textup{(M\arabic*)},leftmargin=*]
  \item\label{item:an17-M1} \textbf{Clause (ii) -- finite part $W_g$
    ...
    seed-Hadamard premise). The honest modulo list is therefore: the
    BFV vanishing, cited only at its carrier's PLAUSIBLE grade
    (Lem.~\ref{an17:Nc-corrected}, revival condition attached), and
    \#B$'$ for patch-traversal consumers; no collar$\to H^s$
...
negative result, discharged iff $(E$-$\mathrm{env})$), the
