...
distance-realization fact (banked, run-11).}
...
(\S\ref{sec:an17-osc}) is banked multiplicity-free, so no per-$\theta$
...
banked $\Xi=-e\cdot G_u$ transversality (R4, fold-blind) and the (R1)/(R4)
curvature data.
\end{proof}
...
\item[\textbf{A.6}$'$] the \textbf{$S1$-transfer / collar bookkeeping}
 (\S\ref{an17:sec-S1transfer}): which banked $S1$ exports restrict to $U_\omega$
...
$\mathrm{fan}_\rho(y)$. The assembly consumes two banked fan-integrated square
...
complement, where $\bar\kappa_2=c_{\mathrm{ch}}^2\kappa_2$. Two banked bounds
...
else in this subsection is the banked $S4$ bookkeeping, unchanged by the class.
...
\item The $S4$ assembly (Propositions~\ref{an17:S4a-D1}, \ref{an17:S4b-profile},
 ...
 $S1$/$S2$/$S3b$ interface} --- the identical relative grade the banked $S2$
...
referee-confirmed status of the companion research record --- the analytic chain
...
\end{equation}
Every arrow is a banked $S3$/$S4$ step. No other constant in the chain sees the
collar: they are all $H^s$-ball facts holding over $U_\omega\subseteq U$ with the
$U$-constants (Proposition~\ref{an17:embed}). Shrinking $\rho_a$ or tightening
...
as the banked $S1$ rung. This subsection justifies that phrase: it inventories the
...
Proposition~\ref{an17:embed}: a holomorphic extension bounded by $\varepsilon_a$
...
\textup{(Prop.~\ref{an17:embed})}. Then every lemma, proposition, and exported
...
scripts \texttt{t1p\_calibrate.py} (with the banked \texttt{ref12} integrator),
...
%     (which is itself THEOREM; the "given" is discharged);
%   * the locked-4d variant an17:slice-ladder-4d at s>23/2 -- THEOREM
%     end-to-end over U_omega GIVEN the same chain.
...
% The collar-margin attainment notes are PLAUSIBLE (rigorous sketch at the
% grade of the banked N0 collar argument; not numerically certified, not
% quoted downstream).
%
...
established in the run-11 trace audit and here restated over the analytic class
...
is the run-11 trace-audit content, THEOREM.
\end{remark}

\begin{lemma}[Transverse restriction of cone profiles to a spacelike slice;
sharp height $\alpha+\tfrac12$]\label{an17:transverse-data}
...
$t<\alpha+\tfrac12$, with constants uniform in $y\in K_\Sigma$ and
$g\in U_\omega$; the exponent $\alpha+\tfrac12$ is \emph{sharp} for the
codimension-one intersection geometry.
\end{lemma}
\begin{proof}
...
which case there is nothing to prove); in $c_0$-uniform tubular coordinates
$(u,\omega)\in(-u_0,u_0)\times\mathcal S_y$ the profile is $u^\alpha\log|u|$
times a $C^1$-bounded factor. The one-dimensional model
$\widehat{u^\alpha\log|u|\,\chi}(\xi)=O(|\xi|^{-\alpha-1}\log|\xi|)$ gives
$\int(1+\xi^2)^{t}|\widehat{\cdot}|^2\,d\xi<\infty$ iff $2t-2\alpha-2<-1$ iff
$t<\alpha+\tfrac12$ --- strict, open, no rounding; tangential smoothness
contributes no loss (finite atlas on $\mathcal S_y$). Near the degenerate limit
(the vertex approaching $\Sigma_0$, $\mathcal S_y$ shrinking) the profile tends
to the radial model $\rho^\alpha\log\rho$ of Lem.~\ref{an17:no-glancing}(ii),
which lies in $H^{t}$ for $t<\alpha+\tfrac32$ --- \emph{more} regular;
monotonicity across the collar gives the $y$-uniform bound at the worse
exponent $\alpha+\tfrac12$.
\end{proof}

\begin{remark}[The half-order correction; what it moves and what it does not]
\label{an17:transverse-note}
The transverse height $\alpha+\tfrac12$ of Lem.~\ref{an17:transverse-data} is
the honest availability of a single-time cone-profile restriction for spectator
points off the intersection surface; the na\"ive radial height $\alpha+\tfrac32$
overstates it by one half-order. This matters for \emph{no} fence in either
register --- the corrected columns are propagated through both ladders below,
and it is precisely the transverse height that makes the $N{=}1$ calibration
(Rem.~\ref{an17:calibration}) reproduce the run-9 wall exactly --- but the
$A4$-type equation-conversion tables of the two ladders should display the
transverse heights $\alpha+\tfrac12$ wherever a single-time profile leg is
booked. This is a fence-neutral bookkeeping tidy, not a proof gap; the
underlying Lem.~\ref{an17:transverse-data} is THEOREM (sharp, $y$-uniform).
\end{remark}
...
\S\ref{an17:sec-data} at their audited grades.} The three scope pins are
...
+2$ on $\mathbb R^4$; its single-time restriction has slice height
$\alpha+\tfrac12$ by Lem.~\ref{an17:transverse-data}). A \emph{slice register}
...
averaged coefficients occurs anywhere; no $+2$ gain anywhere.
...
$|a^{00}|\ge d_\omega$, hence $C_a$-Lipschitz in the collar $C^0$ norm (Weyl
$1$-Lipschitz for $\lambda_{\min}$); the cap
$\varepsilon_a\le\theta_0^{\mathrm{anch}}/(2C_a)$ of Part~1 gives
$\lambda_{\min}(a_g)\ge\tfrac12\theta_0^{\mathrm{anch}}$ for all $g\in U_\omega$,
...
\item[A3.] \emph{Solves with the data-cap column.} Availabilities on $\Sigma_0$
  ...
  respectively (the $\partial_y$-slotted coefficient legs book $+\tfrac12$ in
...
biscalars) plus the banked two-slot continuity lemma (Arendt--Kreuter
...
Lem.~\ref{an17:transverse-data}); $C_{\Delta_2}$ (\textup{(N-c)}, $=0$ on BFV);
...
The same machinery at $N{=}1$ dies, as it must (run-9, referee-confirmed): seed
...
$>2$, reproducing the run-9 wall exactly from the energy side. At $N{=}2$ the
...
transverse height $\alpha+\tfrac12$ of Lem.~\ref{an17:transverse-data} is what
makes the calibration exact.
\end{remark}
...
range mismatch of the run-11 file at $s\ge13$. Second --- and this is the only
...
locked. Both are method-relative upper bounds for the fence (run~6), not
...
coercivity-floor attainment note the two ladders' Rung~1 depends on, at its
honest PLAUSIBLE grade.
...
\item \emph{The $(T1)$-free core --- unconditional THEOREM.} The anchors HB2 and
  ...
  \emph{Grade: THEOREM ($(T1)$-free, $s>8$)} --- the unconditional core, referee
  re-verified.
\item \emph{The anchor rows R0/HB1/HB3 --- THEOREM at their rows, $(T1')$-modulo
  ...
  modulo $T1'$; HB3 $8.5/8$) via the referee-verified restatement of the run-11
...
\end{enumerate}
The historical run-8 ``theorem-modulo'' blanket on the anchor imports of the
bulk clause was localized to R0/HB1/HB3 at run~11; HB2/HB4 Part-A were
...
\begin{remark}[Collar-margin coercivity floor: attainment note]
\label{an17:attainment}
The Rung-1 ellipticity constant $\theta\ge\theta_*=\min(1,\tfrac12
\theta_0^{\mathrm{anch}})>0$ of both ladders is ball-uniform \emph{by the collar
margin, not by attainment on the ball}. This is the honest replacement of the
false ``$H^s$-closed ball, infimum attained'' clause: $U_\omega$ is $H^s$-dense
with empty $H^s$-interior, hence not $H^s$-closed, so no infimum over the ball
is attained. Instead $\theta_0^{\mathrm{anch}}=\min_{K_\Sigma}\lambda_{\min}
(a_{g_0})>0$ is the anchor floor at the compact slab of the fixed analytic
$g_0$ (Chru\'sciel--Grant nondegeneracy \cite{ChruscielGrant2012}), and the
$\varepsilon_a$-cap of Part~1 propagates it to all of $U_\omega$ by the Weyl
$1$-Lipschitz continuity of $\lambda_{\min}$ in the collar $C^0$ norm; only
$1/\theta\le1/\theta_*$ is consumed. \emph{Grade: PLAUSIBLE} --- a rigorous
sketch at the same grade as the banked $N_0$ collar-margin argument of Part~1
that it mirrors; $\theta_0^{\mathrm{anch}}$ and $C_a$ are existence constants,
not numerically certified, and are \emph{not quoted downstream}. The
collar-margin route is what keeps the coercivity floor honest on a class with
empty interior.
\end{remark}
...
(pair $(8,6)$), $R_{\mathrm{sm}}$ strictly softer. The transport average is
...
$W_{g,2}$ on $\Sigma_0$ are finite at the transverse leg heights of
Lem.~\ref{an17:transverse-data}. \emph{Grade: the transport core and its
registers are the banked run-11 \textup{(N-a)} (twice-verified); the
...
coefficient legs at exactly $\tfrac12$, profile legs $\ge\tfrac12$); on the BFV
...
\item \textup{(E-env)} --- \emph{the state-leg kernel envelope.}
  ...
  $N{=}2$-healed ceiling; at $N{=}1$ the ceiling is $j<3$, the run-9 data wall).
...
\item \#A --- \emph{the long-slab traversal.} By Lem.~\ref{an17:DoD} a short slab
  suffices \emph{provided} its data are supplied at the demand levels; the staged
...
derivative-budget methods, each calibrated against two referee-confirmed ground
...
restriction Lem.~\ref{an17:transverse-data}; the slice-register ladder
...
itself a THEOREM. It publishes, at their audited sub-theorem grades and never
...
\#B' (Rem.~\ref{an17:named-residues}); the collar-margin attainment note
(Rem.~\ref{an17:attainment}, PLAUSIBLE); and clause (ii) itself
...
constant-tracking, named-un-derived; clause (i) difference-form, named\}. The
...
% ---------------------------------------------------------------------
\subsection{Clause (i): the singular part (banked single-metric
theorem; difference form a named input)}
...
\end{equation}
ball-uniform for $g,g'\in U_\omega$.  Its banked status splits into a
\emph{single-metric} half that is a theorem in the corpus, and a
\emph{difference} half that is retained as a named input.

\begin{lemma}[Singular jet, single-metric, banked $s>n/2+6$]
...
This is the referee-confirmed ground truth \textbf{GT1} of the
difference-fence adjudication: the single-metric $(1,1)$ jet of $W_g$
is $C^0$ at $s>8$ and fails at $s\le8$ (two independent
symbol-counting methods, calibrated to GT1/GT2, agree).  It is a
\textbf{theorem} at the single-metric level (grade banked in the
early-run record; the difference-fence recomputation reproduces it as
...
\begin{remark}[The difference form of clause~(i) is a \emph{named
input}, not proved here]\label{rem:an17-clausei-diff}
...
banks is the single-metric jet; the family-uniform difference bound is
...
\emph{Grade: NAMED INPUT (difference form); THEOREM (single metric).}
This is the honest scope: this appendix does \emph{not} promote
clause~(i) to a family-uniform theorem.
\end{remark}
...
together with Thm.~\ref{thm:A1-main} (A1, referee-1 confirmed,
adjudicator-reproduced, with the run-16b kernel-lemma completion); (A3)
...
restatements (\S\ref{sec:an17-fence}).  This is the run-16 end-to-end
...
(Rem.~\ref{rem:an17-clauseiii}), (M3) a named difference-input
(Rem.~\ref{rem:an17-clausei-diff}); they are stated here as explicit
hypotheses and printed in the modulo-list (\S\ref{sec:an17-modulo}).
\end{proof}
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=*]
  ...
  \item \emph{The class is $U_\omega$, not an open $H^s$ ball.}
    ...
    clause was struck at run-16b.  Both members $g,g'$ of every
...
\begin{enumerate}[label=\textup{(\alph*)},leftmargin=*]
  \item \emph{The three named clauses themselves.}  (M1) clause~(ii) is
    ...
    and the constant-tracking); (M2) clause~(iii) and (M3) clause~(i)
    difference-form are named inputs, un-derived in print for the family.
...
\begin{enumerate}[label=\textup{(M\arabic*)},leftmargin=*]
  ...
  \item\label{item:an17-M2} \textbf{Clause (iii) -- Sanders
    ...
    $g$, never takes an $H^s$-open modulus.  \emph{Grade: NAMED INPUT}
  ...
  \item\label{item:an17-M3} \textbf{Clause (i) difference form --
    ...
    three).  \emph{Grade: NAMED INPUT} (THEOREM at single-metric level;
    the family-uniform difference bound is part of the named black box).
\end{enumerate}
...
patches), the \#A collar architecture (the long-slab restatement, a
run-12 negative-result, discharged iff $(E$-$\mathrm{env})$), the
$\alpha+\tfrac12$ fence-neutral table repairs (bookkeeping over the
theorem $\mathrm{lem:transverse\text{-}data}$), and the HB1/HB3 imports of
