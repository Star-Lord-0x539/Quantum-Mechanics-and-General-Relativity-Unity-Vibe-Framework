...
exactly the class $\mathcal U$ of Hyp.\ hyp:hadamard-uniform (Option B);
by an17:lem-embed, $U_\omega\subseteq U$ once $\varepsilon_a\le
...
Hadamard parametrix of Lem.~an17:G,
...
Cheeger--Gromov--Taylor, cited at its printed site an17:G, THEOREM;
...
$\ell$ the single global length scale (an17:G(ii)), and
...
single-metric half, established as Lem.~an17:clausei-single); every bound
...
(N-a) block records at l.3219--3222):
...
eq:an17-clausei $=$ clause (i) of Hyp.\ hyp:hadamard-uniform (Option B)
and of Hyp.\ hyp:hadamard-uniform-omega: same pairing, same three norms
...
(M3) asks for (an17:lem-embed). $H_g$ is the appendix's own object
...
Lem.~an17:clausei-single bound with $C_H(K,g)\le
...
This port closes item (M3) of sec:an17-modulo at THEOREM grade: the difference form
...
Consumer effect (per the printed audit, appendix l.3588--3591):
clause (i)-diff feeds only the singular leg of
thm:E2-Lipschitz and no quantitative slice-register consumer, so no
